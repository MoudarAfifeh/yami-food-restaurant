...
import { calculateTotal } from "../utills/localFunction";
...
const CartContainer = () => {
  ...
  const findTotal = (newCartItems) => {
    let total = calculateTotal(newCartItems);
    setTotal(total);
  };
    ...
    >
      ...
      {cartItems && cartItems.length > 0 ? (
        <div className=" w-full h-full bg-cartBg rounded-t-[2rem] flex flex-col">
          {/* cart Items section */}
          <div
            className=" w-full h-340 md:h-[25rem] px-6 py-10 flex flex-col gap-3 overflow-y-scroll
          scrollbar-none"
          >
            {/* cart Item */}
            {cartItems &&
              cartItems.map((item) => (
                <CartItem key={item.id} item={item} findTotal={findTotal} />
              ))}
          </div>
          {/* cart total section */}
          <div
        ...
        <div className=" w-full h-full flex flex-col items-center justify-center gap-6">
          <img src={EmptyCart} alt="empty cart image" className=" w-300" />
