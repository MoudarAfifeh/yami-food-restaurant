...
const HomeContainer = () => {
  return (
    <section
      className=" grid grid-cols-1 md:grid-cols-2 gap-2 w-full lg:h-[calc(100vh-12vh)]"
      id="home"
    >
      <div className=" py-2 flex-1 flex flex-col items-start justify-center gap-8">
        <div
          className=" flex items-center justify-center bg-orange-100 px-4 py-1
         rounded-full gap-2"
        >
      ...
      <div className=" py-2 flex-1 flex items-center relative">
        ...
        />
        <div
          className=" w-full h-full absolute flex top-0 left-0 items-center justify-center
          2xl:px-40 py-4 gap-4 flex-wrap"
        >
          {heroData &&
            heroData.map((n) => (
              <div
           ...
           rounded-3xl flex flex-col items-center justify-center drop-shadow-lg"
              >
                ...
                <p className=" text-sm font-semibold text-headingColor">
                  <span className=" text-xs text-purple-600">$</span> {n.price}
                </p>
              </div>
            ))}
        </div>
